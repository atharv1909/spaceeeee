...
def so3_exp(omega: np.ndarray) -> np.ndarray:
    ...
    if np.any(large):
        ...
        K = skew_symmetric(k)
        # theta[large] keeps the trailing (m,1) shape from the keepdims=True
        # norm above; index out that singleton axis (theta[large, 0] -> (m,))
        # before adding the two broadcast axes, otherwise [:, None, None]
        # produces (m,1,1,1) instead of (m,1,1) and the multiply below
        # broadcasts into an incorrect (m,m,3,3) result.
        theta_large = theta[large, 0]
        sin_t = np.sin(theta_large)[:, None, None]
        one_minus_cos = (1.0 - np.cos(theta_large))[:, None, None]
        I = np.eye(3)[None, ...]
