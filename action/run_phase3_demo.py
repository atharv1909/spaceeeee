...
# Allow running this file directly (`python action/run_phase3_demo.py`) from
# anywhere by ensuring the repo root is on sys.path, then import from the
# real action package (there is no standalone "phase3" module).
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from action import default_spacecraft_config, ActionAgent, CounterfactualVisualizer
...
def main():
    ...
    if result:
        ...
        # NOTE: the visualizer needs full per-horizon Monte-Carlo trajectory
        # arrays, which _format_payload() intentionally strips out of the
        # wire payload before publishing (they're far too large to put on
        # Redis). Use agent.last_results (the raw, un-trimmed
        # evaluate_all_actions() output) instead of result['payload'].
        print("\nGenerating plots...")
        output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'output')
        os.makedirs(output_dir, exist_ok=True)
        save_prefix = os.path.join(output_dir, 'phase3')
        viz = CounterfactualVisualizer()
        viz.plot_all({'all_actions': agent.last_results}, save_prefix=save_prefix)
        print(f"Plots saved to {save_prefix}_*.png")
    else:
