...
class PoseEstimateMessage(BaseMessage):
    ...
    # True if the translation in this message was replaced with a safe
    # fallback value (e.g. because the raw model output was out of a
    # realistic orbital range) rather than being a genuine model estimate.
    # Downstream consumers (cognition, action) should propagate this flag
    # on any message derived from this pose rather than silently treating
    # the fallback value as real.
    pose_is_fallback:   bool  = False
...
class SituationVectorMessage(BaseMessage):
    ...
    # Propagated from the PoseEstimateMessage this situation assessment was
    # derived from — True if it was built on a fallback/synthetic pose.
    pose_is_fallback:   bool  = False
...
class ActionRecommendationMessage(BaseMessage):
    ...
    # Propagated from upstream — True if this recommendation was computed
    # from a fallback/synthetic pose rather than a genuine model estimate.
    pose_is_fallback:     bool  = False
