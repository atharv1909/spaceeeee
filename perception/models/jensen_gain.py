...
class JensenGainMonitor:
    ...
    Thresholds (from outline):
        ...
        25.0 - 40.0  : MODERATE          (yellow)
        G_J >= 40.0 : LOW CONFIDENCE    (red)
    ...
    Calibration note: a textbook symmetry ambiguity (a 50/50 flip between
    two valid poses N degrees apart, e.g. a spacecraft whose solar panels
    make a 90-degree attitude indistinguishable) produces a geodesic
    spread of exactly N/2 degrees around the Frechet mean. For the classic
    90-degree case that's 45 degrees, which must fall in LOW CONFIDENCE
    (this is precisely the failure mode this monitor exists to catch), so
    MODERATE_THRESH must stay below 45.
    """

    # Confidence thresholds in degrees
    HIGH_CONFIDENCE_THRESH = 25.0    # degrees
    MODERATE_THRESH = 40.0          # degrees
