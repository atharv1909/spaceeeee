...
# Make the repo root importable regardless of cwd (matches interface/app.py)
...
# Maps the dashboard's int override level to the string values used on the
# wire (orchestrator/message_schemas.OverrideLevel) and understood by
# orchestrator.py's real ArmstrongProtocol.
_LEVEL_TO_WIRE = {
    1: "acknowledge",
    2: "modify",
    3: "replace",
...
class ArmstrongProtocol:
    """
    Thin client for the Armstrong Protocol.

    This does NOT re-implement override/timeout logic locally -- that lives
    in orchestrator/armstrong_protocol.py::ArmstrongProtocol, running inside
    the Orchestrator process, which subscribes to the "human.in" Redis
    channel and is the single source of truth for override state, timeout
    behavior, and history. This class only publishes a real
    HumanOverrideMessage-shaped payload to that channel (the same thing
    interface/app.py's /api/override endpoint already does) and keeps a
    local, display-only log for this dashboard session.
    """
    def __init__(self, redis_url: Optional[str] = None):
        self.redis_url = redis_url or os.environ.get("REDIS_URL", "redis://localhost:6379/0")
        self.override_log: List[Dict[str, Any]] = []  # local display history only
    ...
    async def initiate_override(
        ...
        if level not in _LEVEL_TO_WIRE:
            raise ValueError("Protocol violation: Unknown safety override level.")

        wire_level = _LEVEL_TO_WIRE[level]
        if level >= 2 and not (rationale and rationale.strip()):
        ...
        msg = {
            ...
            "override_level": wire_level,
            "selected_action": target_action_id,
        ...
        if redis_lib is not None:
            try:
                r = redis_lib.Redis.from_url(self.redis_url)
        ...
        log_entry = {**msg, "published_to_orchestrator": published}
        self.override_log.append(log_entry)

        if not published:
            # Surface the failure rather than silently pretending the
            # override reached anyone -- a crew member needs to know a
            # Level 3/4 command didn't actually make it to the orchestrator.
            return {
