...
def perception_output_to_redis_msg(output, pose_is_fallback: bool = False,
                                   t_override=None) -> PoseEstimateMessage:
    """Convert PerceptionAgent output to orchestrator PoseEstimateMessage.

    Args:
        pose_is_fallback: True if `t_override` (or output.pose.t, if no
            override given) is a safe substitute value rather than a
            genuine model estimate — gets carried on the message so
            downstream consumers know not to trust it as real.
        t_override: if given, replaces output.pose.t in the message (used
            when the raw translation was out of a realistic range).
    """
    return PoseEstimateMessage(
        R=output.pose.R,
        t=t_override if t_override is not None else output.pose.t,
        ...
        pose_is_fallback=pose_is_fallback
    )
...
def hdc_output_to_redis_msg(hdc_result: dict, pose_is_fallback: bool = False) -> SituationVectorMessage:
...
def action_result_to_redis_msg(results: list, pose_is_fallback: bool = False) -> ActionRecommendationMessage:
...
class IntegratedSystem:
    ...
    def run_full_pipeline(self, image: np.ndarray,
        ...
        # Determine once, up front, whether the raw translation is usable.
        # Previously this check ran only at the Action stage (Step 3),
        # which meant Cognition (Step 2) silently consumed the raw,
        # possibly out-of-range translation while only Action got the
        # substituted value -- an inconsistency, and one where the
        # substitution was never flagged to either downstream consumer.
        t_raw = np.array(perc_output.pose.t)
        t_norm = np.linalg.norm(t_raw)
        pose_is_fallback = bool(t_norm < 1e-3 or t_norm > 1000.0)
        t_vec = t_raw if not pose_is_fallback else np.array([10.0, 0.0, 0.0])  # safe default: 10m on R-bar

        perc_msg = perception_output_to_redis_msg(
            perc_output, pose_is_fallback=pose_is_fallback, t_override=t_vec
        )
        ...
        sit_msg = hdc_output_to_redis_msg(hdc_result, pose_is_fallback=pose_is_fallback)
        ...
        action_msg = action_result_to_redis_msg(action_results, pose_is_fallback=pose_is_fallback)
